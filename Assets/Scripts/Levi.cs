...
public class Levi : MonoBehaviour
{
    ...
    void ComputeLevyFlight() {
        // 0. draw two random values following a gaussian distributions
        gaussValues = RandomGaussianMPM();
        u = gaussValues.x;
        v = gaussValues.y;

        // 1. choose a random direction [0, 2*Pi)
        do {
            direction = (float)(rand.NextDouble() * (2 * Math.PI));
        } while (direction == 0);
        

        // 2. determine a step length using mantegnas algorithm 
